...
- Use a module-level docstring to describe what the test is testing, and how it
  is testing it.
- When subclassing the NavcoinTestFramework, place overrides for the
...
with the navcoind(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a navcoind.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the Navcoin Core node application logic. For custom behaviour, subclass the
...
The [`TestShell`](test-shell.md) class exposes the NavcoinTestFramework
...
Taken from the [python-navcoinrpc repository](https://github.com/jgarzik/python-navcoinrpc).
...
Test objects for interacting with a navcoind node over the p2p interface.

#### [script.py](test_framework/script.py)
Utilities for manipulating transaction scripts (originally from python-navcoinlib)
