...
NavcoinGUI::NavcoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
...
void NavcoinGUI::createActions()
{
    ...
    connect(overviewAction, &QAction::triggered, this, &NavcoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &NavcoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &NavcoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Navcoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Navcoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a navcoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Navcoin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &NavcoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &NavcoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &NavcoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &NavcoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &NavcoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &NavcoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &NavcoinGUI::setCurrentWallet);
                    activity->open(path);
                });
            }
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &NavcoinGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &NavcoinGUI::setPrivacy);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL + Qt::SHIFT + Qt::Key_C), this), &QShortcut::activated, this, &NavcoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL + Qt::SHIFT + Qt::Key_D), this), &QShortcut::activated, this, &NavcoinGUI::showDebugWindow);
}
...
void NavcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &NavcoinGUI::setCurrentWalletBySelectorIndex);
...
void NavcoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &NavcoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &NavcoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &NavcoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &NavcoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void NavcoinGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &NavcoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &NavcoinGUI::removeWallet);

    auto activity = new LoadWalletsActivity(m_wallet_controller, this);
    activity->load();
}

WalletController* NavcoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void NavcoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &NavcoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &NavcoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &NavcoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &NavcoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &NavcoinGUI::incomingTransaction);
    connect(this, &NavcoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
    wallet_view->setPrivacy(isPrivacyModeActivated());
    const QString display_name = walletModel->getDisplayName();
    m_wallet_selector->addItem(display_name, QVariant::fromValue(walletModel));
}

void NavcoinGUI::removeWallet(WalletModel* walletModel)
{
    if (!walletFrame) return;
...
void NavcoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame) return;
...
void NavcoinGUI::setCurrentWalletBySelectorIndex(int index)
{
...
void NavcoinGUI::createTrayIcon()
{
...
void NavcoinGUI::createTrayIconMenu()
{
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &NavcoinGUI::toggleHidden);
...
void NavcoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void NavcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Navcoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void NavcoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void NavcoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &NavcoinGUI::quitRequested);
    dlg->setCurrentTab(tab);
    dlg->setModel(clientModel->getOptionsModel());
    GUIUtil::ShowModalDialogAsynchronously(dlg);
}

void NavcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header, SynchronizationState sync_state)
{
...
void NavcoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void NavcoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &NavcoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &NavcoinGUI::show);
                e->ignore();
            }
        }
    }
...
#ifdef ENABLE_WALLET
void NavcoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
...
void NavcoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void NavcoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void NavcoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void NavcoinGUI::updateWindowTitle()
{
...
static bool ThreadSafeMessageBox(NavcoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = NavcoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
bool NavcoinGUI::isPrivacyModeActivated() const
{
      ...
      m_platform_style{platformStyle}
{
    ...
    for (const NavcoinUnits::Unit unit : units)
    {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, NavcoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const NavcoinUnits::Unit u : NavcoinUnits::availableUnits()) {
        menu->addAction(NavcoinUnits::longName(u))->setData(QVariant(u));
    }
...
{
    setText(NavcoinUnits::longName(newUnits));
}
