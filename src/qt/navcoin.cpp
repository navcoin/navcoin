...
static bool InitSettings()
{
    ...
    if (!gArgs.ReadSettingsFile(&errors)) {
        std::string error = QT_TRANSLATE_NOOP("navcoin-core", "Settings file could not be read");
        std::string error_translated = QCoreApplication::translate("navcoin-core", error.c_str()).toStdString();
    ...
    if (!gArgs.WriteSettingsFile(&errors)) {
        std::string error = QT_TRANSLATE_NOOP("navcoin-core", "Settings file could not be written");
        std::string error_translated = QCoreApplication::translate("navcoin-core", error.c_str()).toStdString();
        InitError(Untranslated(strprintf("%s:\n%s\n", error, MakeUnorderedList(errors))));
...
static const char* qt_argv = "navcoin-qt";
...
void NavcoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the NavcoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", NavcoinGUI::DEFAULT_UIPLATFORM);
...
{
    window = new NavcoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &NavcoinGUI::quitRequested, this, &NavcoinApplication::requestShutdown);
...
void NavcoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    ...
    m_splash->show();
    connect(this, &NavcoinApplication::splashFinished, m_splash, &SplashScreen::finish);
    connect(this, &NavcoinApplication::requestedShutdown, m_splash, &QWidget::close);
}

void NavcoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool NavcoinApplication::baseInitialize()
{
...
void NavcoinApplication::startThread()
{
    ...
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &NavcoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, &QCoreApplication::quit);
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &NavcoinApplication::handleRunawayException);
    connect(this, &NavcoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &NavcoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}
...
void NavcoinApplication::InitPruneSetting(int64_t prune_MiB)
{
...
void NavcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success)
    {
        ...
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &NavcoinGUI::handlePaymentRequest);
            connect(window, &NavcoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void NavcoinApplication::handleNonFatalException(const QString& message)
{
...
bool NavcoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", NavcoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
