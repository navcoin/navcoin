...
bool parseNavcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!NavcoinUnits::parse(NavcoinUnits::BTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
QString formatNavcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("navcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(NavcoinUnits::format(NavcoinUnits::BTC, info.amount, false, NavcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openNavcoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", NAVCOIN_CONF_FILENAME));
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("navcoin-%s.desktop", chain);
}
