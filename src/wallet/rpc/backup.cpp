    ...
    if (id == uint256::ONE) {
        ...
        if (!seed_id.IsNull())
        {
            if (keyman->GetKey(seed_id, seed)) {
                ...
                mnemonic_words = create_mnemonic(keyData, string_to_lexicon(language));
                for (auto& word : mnemonic_words) {
                    mnemonic += word + " ";
                }
                mnemonic.pop_back();    // drop the last space.

                UniValue reply(UniValue::VOBJ);
                reply.pushKV("mnemonic", std::string(mnemonic));
