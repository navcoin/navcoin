...
public:
    ...
    std::vector<fs::path> Files() override
    {
        ...
        files.emplace_back(fs::PathFromString(m_dir_path) / fs::PathFromString(m_file_path));
        files.emplace_back(fs::PathFromString(m_dir_path) / fs::PathFromString(m_file_path + "-journal"));
        return files;
    }
