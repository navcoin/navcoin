...
CMnemonic::Words()
{
    ...
    boost::multiprecision::uint512_t ms (entropy_);
    ...
    for (int i = 0; i < MS; ++i)
    {
        boost::multiprecision::uint512_t word (ms);
        word >>= 11 * (MS - i - 1);
        word &= 0b11111111111;      // fixed for all mnemonic string lenghts.
        words[i] = dic->at(word.convert_to<uint16_t>());
    }
