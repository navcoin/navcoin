...
{
    AssertLockHeld(cs_wallet);
    const auto it = mapOutputs.find(outpoint);
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
...

    {
        ...
        // Completely rewrite the wallet file to avoid leaving bits of the
        // unencrypted private key in slack space in the database file.
        GetDatabase().Rewrite();
...
void CWallet::SetSpentKeyState(WalletBatch& batch, const Outid& hash, bool used, std::set<CTxDestination>& tx_destinations)
{
    AssertLockHeld(cs_wallet);
    const CWalletTx* srctx = GetWalletTxFromOutpoint(hash);
    if (!srctx) return;

    CTxDestination dst;
    if (ExtractDestination(srctx->tx->vout[static_cast<size_t>(srctx->GetOutputIndexFromHash(hash))].scriptPubKey, dst)) {
        if (IsMine(dst)) {
...
CWalletOutput* CWallet::AddToWallet(const COutPoint& outpoint, CTxOutRef out, const TxState& state, const UpdateWalletOutputFn& update_wout, bool fFlushOnClose, bool rescanning_old_block, const SyncTxState& state_spent, bool fCoinbase, const uint256& spent_by)
{
    ...
    auto ret = mapOutputs.emplace(std::piecewise_construct, std::forward_as_tuple(outpoint), std::forward_as_tuple(out, state));
    CWalletOutput& wout = (*ret.first).second;
    ...
    //// debug print
    WalletLogPrintf("AddToWallet %s  %s%s %s\n", outpoint.ToString(), (fInsertedNew ? "new" : ""), (fUpdated ? "update" : ""), TxStateString(state));

    // Write to disk
    if (fInsertedNew || fUpdated)
        if (!batch.WriteOutput(outpoint, wout))
            return nullptr;
    ...
    if (!strCmd.empty()) {
        ReplaceAll(strCmd, "%s", outpoint.ToString());
...
CWalletTx* CWallet::AddToWallet(CTransactionRef tx, const TxState& state, const UpdateWalletTxFn& update_wtx, bool fFlushOnClose, bool rescanning_old_block)
{
    ...
    if (IsWalletFlagSet(WALLET_FLAG_AVOID_REUSE)) {
        ...
        for (const CTxIn& txin : tx->vin) {
            ...
            SetSpentKeyState(batch, op.hash, true, tx_destinations);
        }
    ...
    if (!fInsertedNew) {
        if (state.index() != wtx.m_state.index()) {
            ...
            assert(TxStateSerializedIndex<TxState>(wtx.m_state) == TxStateSerializedIndex<TxState>(state));
            assert(TxStateSerializedBlockHash<TxState>(wtx.m_state) == TxStateSerializedBlockHash<TxState>(state));
        }
...
bool CWallet::LoadToWallet(const uint256& hash, const UpdateWalletTxFn& fill_wtx)
{
    ...
    // If wallet doesn't have a chain (e.g when using navio-wallet tool),
    ...
    AddToSpends(wtx);
    for (const CTxIn& txin : wtx.tx->vin) {
        auto prevtx = GetWalletTxFromOutpoint(txin.prevout);
        if (prevtx != nullptr) {
...
bool CWallet::LoadToWallet(const COutPoint& outpoint, const UpdateWalletOutputFn& fill_wout)
{
    const auto& ins = mapOutputs.emplace(std::piecewise_construct, std::forward_as_tuple(outpoint), std::forward_as_tuple(nullptr, TxStateInactive{}));
    CWalletOutput& wout = ins.first->second;
    if (!fill_wout(wout, ins.second)) {
        return false;
    }
    // If wallet doesn't have a chain (e.g when using navio-wallet tool),
    // don't bother to update txn.
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                while (range.first != range.second) {
                    if (range.first->second != tx.GetHash()) {
                        if (!tx.IsBLSCT()) {
                            WalletLogPrintf("Transaction %s (in block %s) conflicts with wallet transaction %s (both spend %s)\n", tx.GetHash().ToString(), conf->confirmed_block_hash.ToString(), range.first->second.ToString(), range.first->first.hash.ToString());
...
{
    for (const CTxIn& txin : tx->vin) {
        auto it = GetWalletTxFromOutpoint(txin.prevout);
        if (it != nullptr) {
            auto mi = mapWallet.find(it->GetHash());
            if (mi != mapWallet.end()) {
                mi->second.MarkDirty();
            }
        }
...
CAmount CWallet::GetDebit(const CTxIn& txin, const isminefilter& filter, const TokenId& token_id) const
{
    {
        LOCK(cs_wallet);
        const auto wtx = GetWalletTxFromOutpoint(txin.prevout);
        if (wtx != nullptr) {
            auto out_ix = wtx->GetOutputIndexFromHash(txin.prevout);
            if (out_ix == -1) return 0;
            if (static_cast<size_t>(out_ix) < wtx->tx->vout.size()) {
                if (wtx->tx->vout[out_ix].HasBLSCTRangeProof() && IsMine(wtx->tx->vout[out_ix]) & filter) {
                    return wtx->tx->vout[out_ix].tokenId == token_id ? wtx->GetBLSCTRecoveryData(txin.prevout).amount : 0;
                } else if (IsMine(wtx->tx->vout[out_ix]) & filter)
                    return wtx->tx->vout[out_ix].nValue;
            }
...
{
    AssertLockHeld(cs_wallet);
    auto wtx = GetWalletTxFromOutpoint(outpoint);
    if (!wtx) {
        return ISMINE_NO;
    }
    return IsMine(wtx->tx->vout[wtx->GetOutputIndexFromHash(outpoint)]);
}
...
bool CWallet::SignTransaction(CMutableTransaction& tx) const
{
    ...
    for (auto& input : tx.vin) {
        const auto wtx = GetWalletTxFromOutpoint(input.prevout);
        if (wtx == nullptr) {
            return false;
        }
        int prev_height = wtx->state<TxStateConfirmed>() ? wtx->state<TxStateConfirmed>()->confirmed_block_height : 0;
        coins[input.prevout] = Coin(wtx->tx->vout[wtx->GetOutputIndexFromHash(input.prevout)], prev_height, wtx->IsCoinBase());
    }
...
void CWallet::CommitTransaction(CTransactionRef tx, mapValue_t mapValue, std::vector<std::pair<std::string, std::string>> orderForm)
{
    ...
    if (!tx->IsBLSCT()) {
        ...
        for (const CTxIn& txin : tx->vin) {
            const wallet::CWalletTx* wtx = GetWalletTxFromOutpoint(txin.prevout);
            CWalletTx& coin = mapWallet.at(wtx->GetHash());
            coin.MarkDirty();
    ...
    if (!gArgs.GetBoolArg("-walletcrosschain", DEFAULT_WALLETCROSSCHAIN)) {
        ...
        if (batch.ReadBestBlock(locator) && locator.vHave.size() > 0 && chain.getHeight()) {
            ...
            if (chain.getBlockHash(0) != locator.vHave.back()) {
                error = Untranslated("Wallet files should not be reused across chains. Restart naviod with -walletcrosschain to override.");
                return false;
            }
        }
    }
...
int CWallet::GetOutputDepthInMainChain(const CWalletOutput& wout) const
{
    AssertLockHeld(cs_wallet);
    if (auto* conf = wout.state<TxStateConfirmed>()) {
        assert(conf->confirmed_block_height >= 0);
        return GetLastBlockHeight() - conf->confirmed_block_height + 1;
    } else if (auto* conf = wout.state<TxStateConflicted>()) {
        assert(conf->conflicting_block_height >= 0);
        return -1 * (GetLastBlockHeight() - conf->conflicting_block_height + 1);
    } else {
        return 0;
...
int CWallet::GetOutputBlocksToMaturity(const CWalletOutput& wout) const
{
    AssertLockHeld(cs_wallet);
    ...
    int chain_depth = GetOutputDepthInMainChain(wout);
    assert(chain_depth >= 0); // coinbase tx should not be conflicted
    return std::max(0, (COINBASE_MATURITY + 1) - chain_depth);
}
...
bool CWallet::IsOutputImmatureCoinBase(const CWalletOutput& wout) const
{
    AssertLockHeld(cs_wallet);

    // note GetBlocksToMaturity is 0 for non-coinbase tx
    return GetOutputBlocksToMaturity(wout) > 0;
}
...
bool DoMigration(CWallet& wallet, WalletContext& context, bilingual_str& error, MigrationResult& res) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (data->watch_descs.size() > 0 || data->solvable_descs.size() > 0) {
        ...
        if (wallet.IsWalletFlagSet(WALLET_FLAG_BLSCT_OUTPUT_STORAGE)) {
            options.create_flags |= WALLET_FLAG_BLSCT_OUTPUT_STORAGE;
        }
