...
BOOST_AUTO_TEST_CASE(sighash_caching)
{
    ...
    const auto amount{static_cast<CAmount>(InsecureRandBits(63))};
    ...
    for (const auto sigversion: {SigVersion::BASE, SigVersion::WITNESS_V0}) {
        ...
        for (int i{0}; i < 10; ++i) {
            hash_types.push_back(i % 2 == 0 ? static_cast<int8_t>(InsecureRand32()) : static_cast<int32_t>(InsecureRand32()));
        }
