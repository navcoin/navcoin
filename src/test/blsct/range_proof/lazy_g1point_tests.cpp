...
BOOST_AUTO_TEST_CASE(test_lazy_g1points_ctor)
{
    ...
    LazyG1Points<T> points1(
        std::vector<Point> { g, g2 },
        std::vector<Scalar> { 1, 2 }
    );

    LazyG1Points<T> points2;
    points2.Add(LazyG1Point<T>(g, 1));
    points2.Add(LazyG1Point<T>(g, 2));
...
{
    auto lazy_g = LazyG1Point<T>(Point::GetBasePoint(), 1);
    auto lazy_g2 = LazyG1Point<T>(Point::GetBasePoint(), 2);
    LazyG1Points<T> lazy_points;
...
{
    auto g = LazyG1Point<T>(Point::GetBasePoint(), 1);
    auto g2 = LazyG1Point<T>(Point::GetBasePoint(), 2);

    LazyG1Points<T> ps1, ps2, ps3;
    ps1.Add(g);
    ps2.Add(g2);
