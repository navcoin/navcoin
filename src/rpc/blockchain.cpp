...
static RPCHelpMan gettxout()
{
    return RPCHelpMan{"gettxout",
        ...
        {
            {"outid", RPCArg::Type::STR, RPCArg::Optional::NO, "The output id"},
        ...
        RPCExamples{
            ...
            "\nView the details\n"
            + HelpExampleCli("gettxout", "\"outid\"") +
            "\nAs a JSON-RPC call\n"
            + HelpExampleRpc("gettxout", "\"outid\"")
                },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    NodeContext& node = EnsureAnyNodeContext(request.context);
    ...
    auto hash{Txid::FromUint256(ParseHashV(request.params[0], "outid"))};
    COutPoint out{hash};
    bool fMempool = true;
    if (!request.params[1].isNull())
...
static RPCHelpMan scanblocks()
{
    return RPCHelpMan{"scanblocks",
        ...
        "This call may take several minutes. Make sure to use no RPC timeout (navio-cli -rpcclienttimeout=0)",
        {
            scan_action_arg_desc,
            scan_objects_arg_desc,
...
static RPCHelpMan loadtxoutset()
{
    return RPCHelpMan{
        ...
        "The result is a usable naviod instance that is current with the network tip in a "
