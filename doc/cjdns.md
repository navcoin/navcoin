# CJDNS support in Navcoin Core

It is possible to run Navcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Navcoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Navcoin network and individual nodes.
...
before it will work with your Navcoin Core node. This is described in steps
...
## Run Navcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Navcoin Core
...
When enabled, this option tells Navcoin Core that it is running in an
...
IPv6 local network. This helps Navcoin Core perform better address management:
...
CJDNS support was added to Navcoin Core in version 23.0 and there may be fewer
...
## CJDNS-related information in Navcoin Core

There are several ways to see your CJDNS address in Navcoin Core:
