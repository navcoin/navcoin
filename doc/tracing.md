# User-space, Statically Defined Tracing (USDT) for Navcoin Core

Navcoin Core includes statically defined tracepoints to allow for more
...
**Note**: Navcoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Navcoin Core
...
To list probes in Navcoin Core, use `info probes` in `gdb`:
...
The `readelf` tool can be used to display the USDT tracepoints in Navcoin Core.
