Navcoin Core version 0.19.0.1 is now available from:

  <https://navcoincore.org/bin/navcoin-core-0.19.0.1/>
...
Navcoin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Navcoin Core on unsupported systems.
...
to open the Navcoin Core .dmg. This is due to new signing requirements
imposed by Apple, which the Navcoin Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/bitcoin/bitcoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Navcoin Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Navcoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `navcoind -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Navcoin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Navcoin Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Navcoin Core. (#14054)

- To eliminate well-known denial-of-service vectors in Navcoin Core,
  ...
  This prevents Navcoin Core from sending the BIP111 NODE_BLOOM service
  ...
  of Navcoin Core and nodes that have manually enabled BIP37 support,
...
- By default, Navcoin Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `navcoin-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Navcoin Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Navcoin Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Navcoin Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Navcoin address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Navcoin Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Navcoin AEAD (jonasschnelli)
...
- #15639 navcoin-wallet tool: Drop libnavcoin_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in navcoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "navcoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of navcoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from navcoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_navcoin to auto generated project files (sipsorcery)
...
- #15773 Add NavcoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_navcoin output (practicalswift)
...
- #15896 feature_filelock, interface_navcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Navcoin Core (luke-jr)
...
- #16277 Suppress output in test_navcoin for expected errors (gertjaap)
...
- #16489 log: harmonize navcoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (navcoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example navcoin.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBNAVCOIN_SERVER (Bushstar)
...
- #16080 build/doc: update navcoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in navcoin.conf (fanquake)
...
- #16621 add default navcoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the navcoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, navcoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/navcoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/navcoin/navcoin/).
