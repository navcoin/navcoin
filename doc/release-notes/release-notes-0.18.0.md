Navcoin Core version 0.18.0 is now available from:

  <https://navcoincore.org/bin/navcoin-core-0.18.0/>
...
`/Applications/Navcoin-Qt` (on Mac) or `navcoind`/`navcoin-qt` (on
Linux).
...
recommended to use Navcoin Core on unsupported systems.
...
10.10. Additionally, Navcoin Core does not yet change appearance when
...
`contrib/init/navcoind.service`, it has been changed to use
`/var/lib/navcoind` as the data directory instead of
`~navcoin/.navcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/navcoind` will
exist has enough space (check using `df -h /var/lib/navcoind`), and
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://navcoincore.org/en/contact/) the Navcoin Core
...
The systemd init file (`contrib/init/navcoind.service`) has been changed
to use `/var/lib/navcoind` as the data directory instead of
`~navcoin/.navcoin`. This change makes Navcoin Core more consistent with
...
- A new [document](https://github.com/bitcoin/bitcoin/blob/master/doc/navcoin-conf.md)
  about the `navcoin.conf` file describes how to use it to configure
  Navcoin Core.

- A new document introduces Navcoin Core's BIP174 [Partially-Signed
  Navcoin Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Navcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Navcoin Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Navcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Navcoin Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart navcoind with the `-deprecatedrpc=generate`
...
- A new `navcoin-wallet` tool is now distributed alongside Navcoin
...
This section describes planned changes to Navcoin Core that may affect
other Navcoin software and services.

- Since version 0.16.0, Navcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Navcoin Core 0.20 (expected about
  a year after 0.18), Navcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Navcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Navcoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Navcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Navcoin Core
...
- A sub-project of Navcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Navcoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Navcoin Core's own implementation, although entropy
  gathered by Navcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Navcoin Core a
...
- On macOS, Navcoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `navcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libnavcoin_server.a` (jnewbery)
...
- #13926 [Tools] navcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like navcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `navcoin-qt -version` output formatting (hebasto)
...
- #11625 Add NavcoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libnavcoin_server` (laanwj)
...
- #15154 Configure: navcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/navcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Navcoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libnavcoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in navcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Navcoin Core (Christewart)
...
- #12246 Bugfix: Only run navcoin-tx tests when navcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_navcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_navcoin` (MarcoFalke)
...
- #14186 navcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_navcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update navcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/navcoin-conf.md (hebasto)
...
- #15354 Add missing `navcoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct navcoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support navcoin folder on OSX (JimmyMow)
