...
Navcoin Core version 0.20.0 is now available from:

  <https://navcoincore.org/bin/navcoin-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `NAVCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Navcoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Navcoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Navcoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Navcoin Core. Navcoin Core logs debug messages
...
- OpenSSL is no longer used by Navcoin Core.  (#17265)

- BIP70 support has been fully removed from Navcoin Core. The
...
- All custom settings configured when Navcoin Core starts are now
...
- The "Start Navcoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://navcoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Navcoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Navcoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitcoin-core/HWI) for
  signing.  Future versions of Navcoin Core should support a GUI option
...
- Navcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.navcoincore.org.  (#17596)
...
- #17068 Always generate `navcoinstrings.cpp` on `make translate` (D4nte)
...
- #17910 Remove double `LIBNAVCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_navcoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBNAVCOIN_SERVER` from bench-Makefile (brakmic)
...
- #17206 Add testcase to simulate navcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest navcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_navcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (navcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone navcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Navcoin Core files (hebasto)
...
- #17596 Change doxygen URL to doxygen.navcoincore.org (laanwj)
