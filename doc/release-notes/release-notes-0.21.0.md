...
Navcoin Core version 0.21.0 is now available from:

  <https://navcoincore.org/bin/navcoin-core-0.21.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  Navcoin
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use Navcoin Core on
unsupported systems.

From Navcoin Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Navcoin Core does not yet change appearance
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  Version 2 hidden services are still fully supported by Navcoin Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Navcoin Core will no longer
...
- A new `navcoin-cli -netinfo` command provides a network peer connections
...
- A new `navcoin-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `navcoin-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `navcoin-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when Navcoin Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `navcoin-wallet` tool which performs the salvage
...
Navcoin Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`navcoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Navcoin Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `navcoin-cli createwallet` or `navcoin-wallet
...
Users who already have a Navcoin Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Navcoin Core.
...
The change to using descriptors changes the default derivation paths used by Navcoin Core
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time Navcoin Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `navcoin.conf`
...
- #19284 Add seed.navcoin.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in navcoin-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add navcoin-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in navcoin-cli (laanwj)
...
### Build system
- #18504 Drop navcoin-tx and navcoin-wallet dependencies on libevent (ryanofsky)
...
- #18297 Use pkg-config in NAVCOIN_QT_CONFIGURE for all hosts including Windows (hebasto)
...
- #18653 add coverage for navcoin-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz Navcoin Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitrenavcoin)
...
- #18817 Document differences in navcoind and navcoin-qt locale handling (practicalswift)
