Navcoin Core version 0.17.0 is now available from:

  <https://navcoincore.org/bin/navcoin-core-0.17.0/>
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `navcoin.conf` file, not inside included files or from
  ...
  as navcoin.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the navcoin.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=navcoin
    test.uacomment=navcoin-testnet
...
BIP 174 Partially Signed Navcoin Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Navcoin transactions that are not fully signed
...
Overall, the construction of a fully signed Navcoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Navcoin transaction (in network format)
...
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When navcoin is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If navcoin is started
...
- #11962 `0a01843` add seed.navcoin.sprovoost.nl to DNS seeds (Sjors)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI navcoin:// (krab)
...
- #13528 `b877c39` Move NavcoinGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libnavcoin-util to libnavcoin-server (ken2812221)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use NavcoinTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to navcoin.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use NavcoinTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use NavcoinTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use NavcoinTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for NavcoinTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_navcoin` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.navcoin is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_navcoin` once (MarcoFalke)
...
- #13374 `56f6936` utils and libraries: checking for navcoin address in translations (kaplanmaxe)
...
- #13346 `60f0358` update navcoin-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust navcoincore.org links (MarcoFalke)
