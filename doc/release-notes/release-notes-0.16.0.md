Navcoin Core version 0.16.0 is now available from:

  <https://navcoincore.org/bin/navcoin-core-0.16.0/>
...
Navcoin Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
navcoin data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile Navcoin Core is now 4.8. No effort will be
...
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest navcoin.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.navcoin.sprovoost.nl (Sjors)
...
- #11621 `1f7695b` [build] Add temp_navcoin_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` navcoin_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.navcoin doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for navcoin-config.h (danra)
