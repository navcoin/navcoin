# PSBT Howto for Navcoin Core

Since Navcoin Core 0.17, an RPC interface exists for Partially Signed Navcoin
...
PSBT is an interchange format for Navcoin transactions that are not fully signed
...
#### Multisig with multiple Navcoin Core instances
...
Navcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `navcoin-cli` is used.
