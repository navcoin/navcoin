...
The data directory is the default location where the Navcoin Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.navcoin/`
macOS    | `$HOME/Library/Application Support/Navcoin/`
Windows  | `%APPDATA%\Navcoin\` <sup>[\[1\]](#note1)</sup>
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Navcoin blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `navcoin.conf`        | User-defined [configuration settings](navcoin-conf.md) for `navcoind` or `navcoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [navcoin.conf](navcoin-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`navcoin-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Navcoin Core:
