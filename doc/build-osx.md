...
These tools must be installed in order to build Navcoin Core from source.
...
### 4. Clone Navcoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Navcoin Core repository to a directory.
...
Navcoin Core includes a GUI built with the cross-platform Qt Framework.
...
You can deploy a `.dmg` containing the Navcoin Core application using `make deploy`.
...
There are many ways to configure Navcoin Core, here are a few common examples:
...
Run the following in your terminal to compile Navcoin Core:
...
## Running Navcoin Core

Navcoin Core should now be available at `./src/navcoind`.
...
``` bash
/Users/${USER}/Library/Application Support/Navcoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Navcoin"

touch "/Users/${USER}/Library/Application Support/Navcoin/navcoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Navcoin/navcoin.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Navcoin/debug.log
```
