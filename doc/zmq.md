...
The Navcoin Core daemon can be configured to act as a trusted "border
router", implementing the navcoin wire protocol and relay, making
...
The ZeroMQ feature in Navcoin Core requires the ZeroMQ API >= 4.0.0
...
using. Navcoind appends an up-counting sequence number to each
