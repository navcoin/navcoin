# Support for Output Descriptors in Navcoin Core

Since Navcoin Core v0.17, there is support for Output Descriptors. This is a
...
- Followed by the actual key, which is either:
  ...
  - [WIF](https://en.navcoin.it/wiki/Wallet_import_format) encoded private keys may be specified instead of the corresponding public key, with the same meaning.
...
on Navcoin's OP_CHECKMULTISIG opcode. To support these, we introduce the
...
transactions related to the multisig. Lastly, it is not recommended to use anything other than a Navcoin Core descriptor wallet to serve as your
...
existing Navcoin Core wallets, a convenience function `combo` is
...
All RPCs in Navcoin Core will include the checksum in their output. Only
