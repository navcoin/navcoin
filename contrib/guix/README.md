# Bootstrappable Navcoin Core Builds

This directory contains the files necessary to perform bootstrappable Navcoin
Core builds.
...
To build Navcoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Navcoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/navcoin-detached-sigs> ./contrib/guix/guix-codesign
```
