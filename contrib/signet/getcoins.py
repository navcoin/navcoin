...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing navcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='navcoin-cli', help='navcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Navcoin address to which the faucet should send')
...
parser.add_argument('navcoin_cli_args', nargs='*', help='Arguments to pass on to navcoin-cli (default: -signet)')

args = parser.parse_args()

if args.navcoin_cli_args == []:
    args.navcoin_cli_args = ['-signet']


def navcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.navcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = navcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = navcoin_cli(['getnewaddress', 'faucet', 'bech32'])
